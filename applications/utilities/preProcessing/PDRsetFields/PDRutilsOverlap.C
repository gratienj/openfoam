  ...
  \\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox
   \\    /   O peration     |
    \\  /    A nd           | www.openfoam.com
     \\/     M anipulation  |
-------------------------------------------------------------------------------
    Original code Copyright (C) 2016 Shell Research Ltd.
...
)
{
    ...
    */
    ...
    double count, a_lblk, b_lblk, perim, dummy;
...
//******?????
                double area = (a2 - a1) * (b2 - b1);
                double rat = dia * dia / area - 1.5;
                if (rat > 0.0)
                {
                    double da = ac - 0.5 * (a1 + a2);
                    double db = bc - 0.5 * (b1 + b2);
                    double dc = std::hypot(da, db);
                    double rat1 = min(max((dc / std::sqrt(area) - 0.3) * 1.4, 0), 1);
                    double drg0 = c_drag(ia,ib).xx();
                    double drg1 = c_drag(ia,ib).yy();
                    double drg = std::hypot(drg0, drg1);
...
)
{
    ...
    double area, a_lblk, b_lblk, dummy, a_centre, b_centre;
    symmTensor2D dum2;
